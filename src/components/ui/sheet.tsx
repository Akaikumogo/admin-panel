...
/** Ant Design Drawer-compatible wrapper */
export function Drawer({
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose?.()}>
      <SheetContent
        side={placement}
        style={{ maxWidth: width, width: typeof width === 'number' ? width : undefined }}
        className="overflow-y-auto"
      >
        {title ? <SheetTitle className="mb-4 pr-8">{title}</SheetTitle> : null}
        {children}
      </SheetContent>
    </Sheet>
  );
